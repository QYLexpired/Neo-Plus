...
type ColoredListsStyle = 'default' | 'vivid';

const defaultInitialHue = 0;
let coloredListsStyle: ColoredListsStyle = 'default';
...
export function initColoredLists(): void {
  ...
  loadConfig().then((config) => {
    if (!isCurrent()) return;
    coloredListsStyle = config['colored-lists-style'] === 'vivid' ? 'vivid' : 'default';
...
function buildSettingsHTML(i18n: Record<string, string>): string {
  const styleOptions = ['default', 'vivid']
    .map(value => `<option value="${value}">${i18n[`coloredListsStyle${value.charAt(0).toUpperCase() + value.slice(1)}`]}</option>`)
    .join('');
        ...
        <div class="config-items">
          <label class="fn__flex b3-label config-item">
            <div class="fn__flex-1 config-item__main">
              <div class="config-name">${i18n.coloredListsStyle}</div>
              <div class="b3-label__text">${i18n.coloredListsStyleTip}</div>
            </div>
            <span class="fn__space"></span>
            <select class="b3-select fn__flex-center fn__size200" id="neo-colored-lists-style">
              ${styleOptions}
            </select>
          </label>
          <label class="fn__flex b3-label config-item">
          ...
          <label class="fn__flex b3-label config-item${fixedHueClass}" id="neo-colored-lists-initial-hue-item">
            ...
            <div class="b3-tooltips b3-tooltips__n fn__flex-center" id="neo-colored-lists-initial-hue-tooltip" aria-label="${initialHue}deg">
              <input class="b3-slider fn__size200" id="neo-colored-lists-initial-hue" min="0" max="360" step="1" type="range" value="${initialHue}">
            </div>
          </label>
        </div>
      </div>
...
export function showColoredListsSettings(): void {
  ...
  const styleSelect = dialog.element.querySelector('#neo-colored-lists-style') as HTMLSelectElement | null;
  ...
  if (styleSelect) styleSelect.value = coloredListsStyle;
  if (ruleSelect) {
