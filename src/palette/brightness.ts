...
export function createBrightnessSliderHTML(i18n?: Record<string, string>): string {
  ...
  return `<div style="display:flex;align-items:center;width:100%;white-space:normal;">
    <div aria-label="${label}：${currentValue.toFixed(2)}" class="b3-tooltips b3-tooltips__n" id="${id}" style="flex:1;">
      <input type="range" class="b3-slider fn__block" id="${id}-input" min="-1" max="1" value="${currentValue}" step="0.01" style="box-sizing:border-box;display:block;">
    </div>
  </div>`;
}
